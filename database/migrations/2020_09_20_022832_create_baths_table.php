...
class CreateBathsTable extends Migration {
    ...
    public function up() {
        ...
        Schema::create('bath', function (Blueprint $table) {
            ...
            $table->bigIncrements('bath_id');

            // # 基本資訊 -----------------------------------------------------------------------------
            $table->string('service_type')->default('洗澡')->comment('服務類型( 基礎、洗澡、美容、住宿 )') ;
            ...
            // # 洗澡 ---------------------
            $table->string('bath_1' , 30 )->nullable()->comment('第一次洗澡');
            $table->string('bath_2' , 30 )->nullable()->comment('第二次洗澡');
            $table->string('bath_3' , 30 )->nullable()->comment('第一次浸泡');
            $table->string('bath_4' , 30 )->nullable()->comment('第三次洗澡');
            $table->string('bath_5' , 30 )->nullable()->comment('第二次浸泡');
            $table->string('bath_6' , 30 )->nullable()->comment('烘 乾');


            // # 加價項目 ---------------------
            $table->string('plus' , 30 )->nullable()->comment('加價項目 : 美容');

            $table->string('fur' , 30 )->nullable()->comment('加價項目 : 廢毛( 中、重 )');
            $table->string('flea' , 30 )->nullable()->comment('加價項目 : 跳蚤、壁蝨');


            // # 各種價格 ----------------------------
            $table->integer('bath_price')->default(0)->comment('價格 : 美容單_消費價格') ;  // 洗澡單價格
            $table->integer('basic_fee')->nullable()->comment('基礎單_消費價格') ;               // 基礎單價格

            $table->integer('plus_price')->nullable()->comment('加價美容') ;                     // 加價美容
            ...
            // 工作人員 ----------------------------
            $table->string('admin_user' , 30 )->nullable()->comment('工作人員') ;
        ...
        //  幫 Table 寫入註解
        DB::statement("ALTER TABLE `bath` comment '洗澡單資料'");
